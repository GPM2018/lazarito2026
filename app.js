...
async function createReportImage(record) {
  ...
  const panelY = photoHeight + 55;
  context.fillStyle = "#eef9ed";
  ...
  context.fillText("CONSULTA DE LOCAL DE VOTACIÓN", 48, panelY + 60);
  context.fillStyle = "#168044";
  context.font = "700 22px Arial";
  context.fillText("REGISTRO ENCONTRADO", 48, panelY + 97);
...
shareWhatsapp.addEventListener("click", async () => {
  ...
  try {
    ...
    const title = "Resultado de consulta - Lazarito Ovelar 2026";
    const shareData = { title, text: "Resultado de consulta electoral.", files: [file] };
    ...
    const text = `Resultado de consulta\nCI: ${lastConsultation.cedula}\nNombre: ${lastConsultation.name}\nLocal: ${lastConsultation.local}\n${url}`;
